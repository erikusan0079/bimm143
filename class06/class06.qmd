...
Start with a simplified version ofthe problem:
...
How can I remove NA from the vector?
```{r}
!c(F,F,F)
#y[is.na(y)]
```
...
Last step now I have my workig code snippet is to make my `grade()`  function.
...
A: In terms of mean method (sensitive to outliers), HW3 was the toughest on students, however, HW2 was the toughest if we were to use the sum method (insensitive to outliers).
